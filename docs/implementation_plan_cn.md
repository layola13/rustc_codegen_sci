...
3. 当前增量已完成直接标量/raw pointer/void `extern "C"` 调用、null pointer、pointer `Eq`/`Ne`、thin `PtrToPtr` copy，以及 Ignore/Direct ABI worker 校验；下一步做 20-30 个 ABI fixture，优先 Pair/Cast/Indirect/sret/byval。
