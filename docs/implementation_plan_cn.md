...
- 更完整的 basic block、multi-way switch、assert、direct call、标量
  `extern "C"` function pointer indirect call、return。
...
2. 已升级到 `PLAN_VERSION = 12`，包含完整 x86_64 Linux target descriptor（object format、rustc DataLayout、CPU/features、relocation/code model）、`TypeLayoutRecipe`（size/align、field shape、variant/tag encoding、largest niche、scalar valid ranges）、模块级 extern function plan、void return/call plan、raw pointer direct ABI 类型、标量 raw-pointer `Load`/`Store` memory op、标量 stack allocation、`CallIndirect` 显式签名 terminator，以及 rustc-derived `FnAbiPlan`（layout、calling convention、variadic/unwind、Ignore/Direct/Pair/Cast/Indirect pass mode tags）。
3. 当前增量已完成直接标量/raw pointer/void `extern "C"` 调用、标量 `extern "C"` function pointer indirect call、null pointer、pointer `Eq`/`Ne`、thin `PtrToPtr` copy、简单标量 raw-pointer deref load/store、raw-pointer 后的 scalar field offset load/store、标量 stack allocation 及其 size/alignment 校验，以及固定数组下标 scalar load/store、完整 target descriptor 和 type layout worker 校验、backend lowering 错误的 MIR block/statement 定位和 rustc source span、backend fatal 的稳定 `SCI_BACKEND_*` code、worker/backend 共享的 protocol-level `DiagnosticPayload`、backend FnAbi preflight 的 Pair/Cast/Indirect pass-mode 负向 smoke 覆盖、双向单字段 signed/unsigned 8/16/32/64-bit Cast aggregate argument/return 的 linked C smoke、双寄存器 Pair/wide Cast aggregate argument 的 linked C smoke，以及 Ignore/Direct ABI worker 校验和 Pair/Cast/Indirect 负向测试；下一步扩展 ABI fixture，优先 aggregate return、Indirect/sret/byval。
